...
void Image::fd_conv_helper(std::complex<double> img[], uint8_t channel, uint32_t ker_w, uint32_t ker_h, double ker[], uint32_t cr, uint32_t cc) {
  std::complex<double>* _ker = new std::complex<double>[psize];
  pad_kernel(ker_w, ker_h, ker, cr, cc, _ker);

  std::complex<double>* imgFD = new std::complex<double>[psize];
  std::complex<double>* kerFD = new std::complex<double>[psize];
  ...
  std::complex<double>* resFD = new std::complex<double>[psize];
  pointwise_mult(psize, imgFD, kerFD, resFD);

  delete[] imgFD;
  delete[] kerFD;

  std::complex<double>* res = new std::complex<double>[psize];
...
Image& Image::fd_convolve_clamp_to_0(uint8_t channel, uint32_t ker_w, uint32_t ker_h, double ker[], uint32_t cr, uint32_t cc) {
  ...
  std::complex<double>* img = new std::complex<double>[psize];
  for(uint32_t i=0; i<h; ++i) {
    for(uint32_t j=0; j<w; ++j) {
      img[i*pw+j] = std::complex<double>(data[(i*w+j)*channels+channel],0);
    }
  }
  fd_conv_helper(img, channel, ker_w, ker_h, ker, cr, cc);
